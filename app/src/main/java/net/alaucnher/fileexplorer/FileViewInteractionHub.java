...
import net.alaucnher.fileexplorer.FileListItem.ModeCallback;
import net.alaucnher.fileexplorer.FileOperationHelper.IOperationProgressListener;
import net.alaucnher.fileexplorer.FileSortHelper.SortMethod;
import net.alaucnher.fileexplorer.FileViewActivity.SelectFilesCallback;
import net.alaucnher.fileexplorer.TextInputDialog.OnFinishListener;
...
public class FileViewInteractionHub implements IOperationProgressListener {
    ...
    private void notifyFileSystemChanged(String path) {
        Log.v(LOG_TAG, "notifyFileSystemChanged = " + path);
        if (path == null)
            return;
        final File f = new File(path);
        final Intent intent;
        if (f.isDirectory()) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {//如果是4.4及以上版本
    ...
    private net.alaucnher.fileexplorer.FileViewInteractionHub.Mode mCurrentMode;
